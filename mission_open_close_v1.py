"""
mission_open_close_v1.py
完整流程——开环(物料) + 开环+闭环(靶标) 混合版本

与 mission_open_close.py 的区别:
  - 靶标指向: 先开环粗略指向，再闭环PD精准锁定
  - 移除"电机回零"步骤（开环已确保激光在靶标附近，闭环可直接从该位置开始追踪）
  - 闭环超时可适当缩短（起始位置更近）

工作流:
  1. 物料(rect_recognize) 在整个程序运行期间不变，仅在电机初始化前识别一次
  2. 靶标: 开环NN预测(set_position)粗略指向 → 闭环PD速度控制精准锁定
...
MATERIAL_PAN_OFFSET_RIGHTMOST = 1.0  # 最右侧物料额外pan补偿(度)

ROI_SCALE = 1.75          # 激光ROI圆盘半径倍数 (1.7/1.8/1.9)
LASER_V_THRESH = 225     # 激光V通道阈值 (HSV明度, 对红光响应弱于灰度)
LASER_GRAY_THRESH = 232  # 激光灰度阈值 (灰度图亮度门限)

# 闭环PD控制参数
# KP: 像素误差→电机速度的增益。太小则初始响应弱，加速度限制器会累积速度导致overshoot
# KD: 误差变化率→阻尼。太小时接近目标刹不住车
# MAX_ACCEL: 设大一些，避免加速度限制成为伪积分器
KP_PAN = 0.15
KP_TILT = 0.15
KD_PAN = 0.06
KD_TILT = 0.06
...
MAX_SPEED = 200
MAX_ACCEL = 300
CL_TIMEOUT = 10.0         # 单靶标闭环超时(秒)，开环已靠近故可缩短
CL_SETTLE_FRAMES = 15     # 连续在靶帧数才算稳定
TARGET_TRACK_TIMEOUT = 5.0  # 追靶标超时(秒)，超时未锁定则跳过
TR_MEDIAN_WINDOW = 3       # 靶标滤波窗口(帧)，越小响应越快(默认5)
...
def point_to_target_closed_loop(mc, tr_v2, lr_v3, target_color, cam,
    ...
    调用前已通过开环 set_position 将激光大致移到靶标附近，
    此函数从当前激光位置出发，PD追踪靶标中心直至稳定。
    ...
    while time.time() - start_time < timeout:
        ...
        if not track_ok or target_pos is None:
            # 靶标丢失 → 从零重新识别
            ...
            t0_relock = time.time()
            relock_ok = False
            while time.time() - t0_relock < 3.0:
                rf = cam.read_rgb_frame()
                if rf is None:
                    continue
                rg, _ = tr_v2.preprocess(rf)
                re = tr_v2.preprocess_edges(rg)
                rft = tr_v2.find_target_candidates(re)
                rid = tr_v2.identify_colors(rf, rft)
                if tr_v2.validate_geometry(rid):
                    tr_v2.build_template(rf, rid)
                    relock_ok = True
                    break
                cv2.waitKey(1)
            if relock_ok:
                # 重置激光追踪状态
                lr_v3._median_x.reset()
                lr_v3._median_y.reset()
                lr_v3.tracked_candidates.clear()
                lr_v3._frame_count = 0
                print("  靶标重新识别成功，继续闭环...")
                continue
            else:
                print("  靶标重新识别失败")
        ...
        if on_target_count >= settle_frames:
            try:
                ...
                pass
            print(f"  闭环精准锁定完成! 最终误差({prev_ex},{prev_ey})px")
    ...
    try:
        ...
        pass
    print(f"  闭环精准锁定超时/中断 (on_target={on_target_count}/{settle_frames})")
    return False
...
def execute_sequence(ctrl, tr_v2, lr_v3, sequence, materials):
    """按QR码顺序执行：物料(开环) → 靶标(开环粗略 + 闭环精准)。

    核心变化(vs mission_open_close.py):
      - 靶标阶段：先开环 point_to 粗略定位，再闭环PD精准锁定
      - 移除物料→靶标之间的"电机回零"步骤
      - 开环定位后激光已在靶标附近，闭环可直接追踪
    ...
    for i, color in enumerate(sequence):
        ...
        # 找出最右侧物料(横坐标最大)，用于额外pan补偿
        rightmost_cx = max(cx for cx, _ in materials.values()) if materials else 0

        # ==========================================
        # Step A: 指向物料 —— 开环 (水平=中心x, 垂直=底部y+h)
        # ==========================================
        if materials and color in materials:
            ...
            try:
                ...
                if mat_cx == rightmost_cx:
                    pan += MATERIAL_PAN_OFFSET_RIGHTMOST
                    print(f"  -> 最右侧物料: pan额外补偿{MATERIAL_PAN_OFFSET_RIGHTMOST:+.1f}°")
        ...
        # ==========================================
        # Step B: 重新识别靶标(从零) + 开环粗略定位
        # ==========================================
        print(f"  重新识别靶标(从零)...")
        tgt_cx, tgt_cy = None, None
        targets_ok = False
        t0_reinit = time.time()
        while time.time() - t0_reinit < TARGET_TRACK_TIMEOUT:
            frame = ctrl.cam.read_rgb_frame()
            if frame is None:
                continue
            gray, _ = tr_v2.preprocess(frame)
            edges = tr_v2.preprocess_edges(gray)
            final_targets = tr_v2.find_target_candidates(edges)
            identified = tr_v2.identify_colors(frame, final_targets)
            if tr_v2.validate_geometry(identified):
                tr_v2.build_template(frame, identified)
                targets_ok = True
                break
            cv2.waitKey(1)

        if targets_ok and color in tr_v2.ref_targets_centers:
            tgt_cx, tgt_cy = tr_v2.ref_targets_centers[color]
            print(f"  指向{cn}靶标(开环粗略) ({tgt_cx}, {tgt_cy})")
            try:
            ...
            print(f"  ! 靶标重新识别失败，跳过{cn}靶标开环定位")

        print(f"  等待{cn}靶标开环稳定...")
        time.sleep(SETTLE_TIME)

        # ==========================================
        # Step C: 从零重新识别靶标 + 闭环精准锁定
        #   (开环已移动电机，重新识别保证模板匹配当前视角)
        # ==========================================
        print(f"  重新识别靶标(从零)...")
        targets_ok_c = False
        t0_reinit_c = time.time()
        while time.time() - t0_reinit_c < TARGET_TRACK_TIMEOUT:
            frame = ctrl.cam.read_rgb_frame()
            if frame is None:
                continue
            gray, _ = tr_v2.preprocess(frame)
        ...
        if not targets_ok_c:
            print(f"  ! 靶标重新识别超时({TARGET_TRACK_TIMEOUT}s)，跳过{cn}靶标")
        else:
            print(f"  靶标重新识别成功，启动闭环...")
            cl_success = point_to_target_closed_loop(
                ctrl.mc, tr_v2, lr_v3, color, ctrl.cam,
                timeout=TARGET_TRACK_TIMEOUT, settle_frames=CL_SETTLE_FRAMES,
            )
            if not cl_success:
                print(f"  闭环未锁定")
...
def init_targets_and_laser(ctrl):
    ...
    tr_v2 = TargetRecognizerV2(median_window=TR_MEDIAN_WINDOW)
    ...
    lr_v3 = LaserRecognizerV3(
        thresh_val=LASER_GRAY_THRESH, blur_size=3, depth_max=1300,
        roi_scale=ROI_SCALE, v_thresh_val=LASER_V_THRESH,
    )
