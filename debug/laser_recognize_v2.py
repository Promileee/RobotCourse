...
使用方法:
    ...
    from laser_recognize_v2 import LaserRecognizerV2
...
class LaserRecognizerV2:
    ...
    def init_from_target_data(self, target_centers, depth_map):
        ...
        if self.target_depth is not None:
            print(f"[LaserV2] 靶标平面深度: {self.target_depth:.1f}mm (容差 ±{self.depth_tolerance:.0f}mm)")
        else:
            print("[LaserV2] 警告: 未能从靶标中心读取到有效深度值")
        print(f"[LaserV2] 搜索ROI: 圆心({self.roi_center[0]}, {self.roi_center[1]}), "
    ...
    def detect_candidates(self, frame, dpt):
        ...
        for cnt in contours:
            ...
            # 深度检查 — 宽松预过滤，严格的在评分中体现
            d_val = float(dpt[cy, cx]) if (0 <= cy < h and 0 <= cx < w) else -1.0
            if d_val <= 0:
                continue
            if self.target_depth is not None:
...
if __name__ == "__main__":
    ...
    cv2.createTrackbar("Threshold", "Laser Result", 240, 255, lambda _: None)
    cv2.createTrackbar("V-Threshold", "Laser Result", 200, 255, lambda _: None)
    cv2.createTrackbar("Blur", "Laser Result", 3, 10, lambda _: None)
    cv2.createTrackbar("ROI Scale x10", "Laser Result", 17, 30, lambda _: None)
    cv2.createTrackbar("Circularity x100", "Laser Result", 45, 95, lambda _: None)
    cv2.createTrackbar("Depth Tol mm", "Laser Result", 10, 100, lambda _: None)
